...
export default function OutlookMatrix({ outlook }: { outlook: Outlook }) {
  if (!outlook) return <p style={{ color: 'var(--muted)' }}>No outlook data.</p>
  ...
  if (siteNames.length === 0) return <p style={{ color: 'var(--muted)' }}>No outlook data.</p>
  ...
  return (
    <table className="outlook-matrix">
      <thead>
        <tr>
          <th>Site</th>
